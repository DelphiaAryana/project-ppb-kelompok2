...
public class inputFormWK extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_input_form_wk);
...
//                mengubdate database berdasarkan key
                database.child("Wisata Kuliner").child(key).updateChildren(updates);
                finish();
            });
            ...
            simpan.setOnClickListener(v->{
                ...
                lokasi = TVlokasi.getText().toString();
                deskripsi = TVdeskripsi.getText().toString();
                WisataKulinerModel wisataKuliner = new WisataKulinerModel("", nama, lokasi, deskripsi);

                if(filePath != null){
                    wisataKuliner.setImage(getFileName(filePath));
                    uploadImage(filePath);
                }
                submitWisataKuliner(wisataKuliner);
                finish();
            });
        }
    }
    ...
    public void submitWisataKuliner(WisataKulinerModel wisataKuliner){
        database.child("Wisata Kuliner").push().setValue(wisataKuliner).addOnSuccessListener(this, new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void unused) {
                Toast.makeText(inputFormWK.this, "Berhasil tambah data", Toast.LENGTH_SHORT).show();
            }
        });
    }
