...
public class WisataKulinerAdmin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_wisata_kuliner_admin);
        listView = findViewById(R.id.itemsList);
        FAB = findViewById(R.id.tambahData);
        getSupportActionBar().setTitle("Wisata Kuliner Kalsel");

//        menginsiasi database Firebase
        database = FirebaseDatabase.getInstance().getReference();
        populateDataWisataKuliner();
        FAB.setOnClickListener(v -> {
            Intent intent = new Intent(WisataKulinerAdmin.this, inputFormWK.class);
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.detail:
                Intent intent = new Intent(WisataKulinerAdmin.this, DeskripsiWisataKuliner.class);
                intent.putExtra("key", wisataKuliner.getkey());
                startActivity(intent);
                return true;
            case R.id.edit:
                Intent intentUpdate = new Intent(WisataKulinerAdmin.this, inputFormWK.class);
                intentUpdate.putExtra("key", wisataKuliner.getkey());
                startActivity(intentUpdate);
                return true;
            case R.id.hapus:
                showConfirmationDialog(wisataKuliner.getkey());
                populateDataWisataKuliner();
                deleteImageFromStorage(wisataKuliner.image);
                return true;
            default:
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                        database.child("Wisata Kuliner").child(key).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void unused) {
                                Toast.makeText(WisataKulinerAdmin.this, "Data dihapus!", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                })
    ...
    private void deleteImageFromStorage(String imageName) {
        ...
        imageRef.delete().addOnSuccessListener(new OnSuccessListener<Void>() {
            ...
            public void onSuccess(Void unused) {
                ...
                Toast.makeText(WisataKulinerAdmin.this, "Gambar dihapus dari storage", Toast.LENGTH_SHORT).show();
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                // Gagal menghapus gambar dari storage
                Toast.makeText(WisataKulinerAdmin.this, "Gagal menghapus gambar dari storage", Toast.LENGTH_SHORT).show();
            }
        });
    }
}