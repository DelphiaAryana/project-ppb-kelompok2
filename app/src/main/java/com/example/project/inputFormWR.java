...
//                mengubdate database berdasarkan key
            ...
            simpan.setOnClickListener(v->{
                ...
                lokasi = TVlokasi.getText().toString();
                deskripsi = TVdeskripsi.getText().toString();
                WisataReligiModel wisataReligi = new WisataReligiModel("", nama, lokasi, deskripsi);

                if(filePath != null){
                    wisataReligi.setImage(getFileName(filePath));
                    uploadImage(filePath);
                }
                submitWisataReligi(wisataReligi);
                finish();
            });
        }
    }
    ...
    public void submitWisataReligi(WisataReligiModel wisataAlam){
        database.child("Wisata Religi").push().setValue(wisataAlam).addOnSuccessListener(this, new OnSuccessListener<Void>() {
            @Override
